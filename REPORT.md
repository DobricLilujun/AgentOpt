...
- **LLM-driven evolution mode**: an LLM "brain" proposes candidate strategies each
  generation (scored by the EvaluationAgent). It converges to the **same optimum as
  the GA** (6 clusters, 87.5 %, 0 violations) over 10 generations — a genuine
...
- **Public benchmark (Solomon C101 VRPTW)**: the exact ILP returns **15 vehicles —
  ...
  ILP wins (15 vs 92), which is the honest, expected outcome; the multi-agent's niche
  is large/long-horizon and multi-objective problems the exact ILP cannot handle
  (Section 2). The multi-agent is a **complement to the exact ILP, not a replacement**.