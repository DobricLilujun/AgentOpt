...
The system is a team of cooperating agents that, given a set of GR (gas-refilling)
...
- PredictionAgent  (agents.prediction)  : estimate per-unit degradation rate alpha
                                          and the original GR trigger time t_n.
...
- EvaluationAgent  (agents.evaluation)  : score a schedule on cost, #clusters,
                                          SF6 leakage, and reliability (does any
                                          unit drop below the 3.0 bar safety
                                          threshold?), and a composite fitness.
